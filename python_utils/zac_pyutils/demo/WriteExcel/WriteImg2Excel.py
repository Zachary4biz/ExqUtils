...
for fp in pic_fp_list: 
    ...
    img_row, img_col = 12+cnt//2*(16+2), 1+cnt % 2*9
    # print("cnt-{} 图片坐标:".format(cnt), (img_row, img_col))
    ws1.insert_image(row=img_row, col=img_col, filename=f_name,
                     options={'image_data': get_image_as_bytes(img)})
    # 图片的名字填在这里：注意这里下标是以0开始的 12 是图片放置的坐标(这里是0开始的，如果excel里看是第13行)
    first_row, first_col = 12 + 16 + cnt // 2 * (16 + 2), 1 + cnt % 2 * 9
    last_row, last_col = first_row + 2 - 1, first_col + 7 - 1  # 右下角的坐标相比左上角，高2宽7，为了得到坐标要减去1
...
################
# 竖着左边
for i in range(11, reservedRowsFrom + 1):
    ws1.write(i, 0, "", left_border_format)
# 竖着右边
for i in range(11, reservedRowsFrom + 1):
    ws1.write(i, 17, "", right_border_format)
...
for i in range(0, 17 + 1):
    ws1.write(reservedRowsFrom, i, "", bottom_border_format)
# 连接处要改一下
ws1.write(reservedRowsFrom, 17, "", anchor_right_bottom_format)  # 右下角
ws1.write(reservedRowsFrom, 0, "", anchor_left_bottom_format)  # 左下角
